...
import { HighlightNumberInput } from '../ui/HighlightNumberInput';
import { NumberTextInput } from '../ui/NumberTextInput';
...
export const ForgingCalculatorForm = () => {
  ...
  useEffect(() => {
    if (activeMaterials.length > 0 && forging.selected_material_id) {
      const mat = activeMaterials.find(m => m.id === forging.selected_material_id);
      if (mat) {
        if (forging.DG_steel !== mat.latest_price || forging.DG_scrap !== mat.scrap_price) {
          setForgingField('DG_steel', mat.latest_price || 0);
          setForgingField('DG_scrap', mat.scrap_price || 0);
        }
      }
    }
  }, [forging.selected_material_id, activeMaterials, setForgingField]);
  ...
  const C_heat_induction = ((forging.m_chi || 0) || 0) * (forging.w_elec_kwh_per_kg || 0) * (forging.DG_elec_kwh || 0);
  ...
  const C_clean = ((forging.m_chi || 0) || 0) * (forging.DG_clean_kg || 0);
          ...
          <div className="space-y-3 text-xs">
            ...
            <div className="flex items-center justify-between gap-1">
              ...
              <HighlightNumberInput
                min="0"
                step="0.1"
                value={forging.k_mgmt_mat}
                onChange={(v) => setForgingField('k_mgmt_mat', Math.max(0, v))}
              />
            </div>
            ...
            <div className="flex items-center justify-between gap-1">
              ...
              <HighlightNumberInput
                min="0"
                value={forging.d_cut}
                onChange={(v) => setForgingField('d_cut', Math.max(0, v))}
              />
            </div>
            ...
            <div className="flex items-center justify-between gap-1">
              ...
              <HighlightNumberInput
                min="0"
                value={forging.l_cut}
                onChange={(v) => setForgingField('l_cut', Math.max(0, v))}
              />
            </div>
            ...
            <div className="flex items-center justify-between gap-1">
              <div className="flex flex-col">
                ...
                {refWeight > 0 && ((forging.m_chi || 0) || 0) < refWeight && (
              ...
              <HighlightNumberInput
                min="0"
                step="0.0001"
                value={(forging.m_chi || 0)}
                hasError={refWeight > 0 && ((forging.m_chi || 0) || 0) < refWeight}
                onChange={(v) => setForgingField('m_chi', Math.max(0, v))}
              />
            </div>
            ...
            <div className="flex items-center justify-between gap-1">
              ...
              <HighlightNumberInput
                ...
                onChange={(v) => setForgingField('m_phoi', Math.max(0, v))}
              />
            </div>
            ...
            <div className="flex items-center justify-between gap-1 bg-[#FBFBFA] p-1.5 rounded-[4px] border border-[#EAEAEA]">
              <div className="flex flex-col">
                <label className="flex items-center gap-1 cursor-pointer">
                  ...
                  />
                  <span className="text-[10px] font-bold text-[#787774] uppercase tracking-wider">8. Tính theo TL tinh:{forging.use_m_tinh && <span className="text-amber-600 ml-0.5">*</span>}</span>
                </label>
                {(forging.m_phoi || 0) > 0 && (forging.m_tinh || 0) > (forging.m_phoi || 0) && (
                  <span className="text-[10px] text-red-500 font-normal">⚠️ Tinh không được lớn hơn TL Phôi ({(forging.m_phoi || 0)}kg)</span>
                )}
              </div>
              <HighlightNumberInput
                min="0"
                ...
                hasError={(forging.m_phoi || 0) > 0 && (forging.m_tinh || 0) > (forging.m_phoi || 0)}
                onChange={(v) => setForgingField('m_tinh', Math.max(0, v))}
                className={!forging.use_m_tinh ? "bg-gray-100 text-gray-400 cursor-not-allowed border-gray-200" : ""}
              />
            </div>
            ...
            <div className="flex items-center justify-between gap-1">
              ...
              <NumberTextInput
                ...
                onChange={(v) => setForgingField('k_loss', Math.max(0, v))}
        ...
        mainRightContent={
          <div className="space-y-1.5 text-xs font-mono">
            ...
            <div className="flex justify-between items-center py-0.5 border-b border-[#EAEAEA]">
              ...
              <div className="font-bold text-[#111111]">{((forging.m_chi || 0) * forging.DG_steel * (1 + (forging.k_mgmt_mat || 0) / 100)).toLocaleString('vi-VN')} VNĐ</div>
            </div>
          ...
          <div className="space-y-3 text-xs">
            ...
            <div className="flex items-center justify-between gap-1">
              ...
              <NumberTextInput
                ...
                onChange={(v) => setForgingField('C_coining_per_unit', Math.max(0, v))}
                className="w-20 px-2 py-1 border border-[#EAEAEA] rounded-[4px] font-mono text-xs font-bold text-[#111111] text-right"
              />
            </div>
          </div>
